...
----
.. Select the *Add a media type* link.
.. In the dropdown, select the *text/xml* option and click on *Add*.
...
.. Click *Create App*.
.. On the following screen under the *Add features and functionality* section, select *Incoming Webhooks*.
...
.. Under *Where should Fuse Online Demo post?*, select *low-code-demo* in the *Search for a channel* field, and click *Allow*.
+
