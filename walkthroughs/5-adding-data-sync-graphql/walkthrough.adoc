...
. Click the *From Catalog* button.
. Search the developer catalog for 'Data Sync Showcase'.
. Click on the 'Data Sync Showcase' entry to add it to your project.
. Select "Instantiate Template"
. Accept all the defaults of the provisioning form and click *Create*.
. Go to link:{openshift-host}/k8s/ns/{walkthrough-namespace}/pods screen of your project and wait until three pods are running:
+
...
. Go to the link:{route-ionic-showcase-server-host}/graphql[GraphQL playground, window="_blank"] and replace the text in the left pane of the screen with the following query and mutation:
