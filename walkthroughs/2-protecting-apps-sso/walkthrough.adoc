...
//attributes
:title: 2 - Protecting applications using single sign-on - testing
...
. Go to the link:{sso-realm-url}[Master, window="_blank", id="{task-context}-1"] realm, which is running on your {customer-sso-name} service.
...
. Go to link:{sso-user-realm-url}[SSO Realm, window="_blank", id="{task-context}-1"].
. If prompted, log in with your username and password.
...
. Login to the link:{openshift-console-url}[OpenShift Console, window="_blank", id="{task-context}-2"].
...
+
NOTE: the pipe symbol (`|`) allows for multiline input, see link:https://yaml.org/spec/1.2/spec.html#id2795688[the yaml spec, id="{task-context}-3"] for more details.
...
. Log in to the link:{openshift-console-url}[OpenShift Console, window="_blank", id="{task-context}-4"].
. Select the project that contains the pods from *{sp1-title}*.
